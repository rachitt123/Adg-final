...
struct AIFeatures: View {
    ...
    private func classifyImage(_ image: UIImage) {// receives a ui image
        guard let ciImage = CIImage(image: image) // easier to pass to vision api
        else { return }
        
        do {
            let model = try VNCoreMLModel( // vision doesnt directly work with ml model  so needs a vm core model
                for: img_lesser(configuration: MLModelConfiguration()).model
            )
            
    let request = VNCoreMLRequest(model: model) { request, _ in  // Vision request that runs a Core ML model on an image
                if let results = request.results as? [VNClassificationObservation], // request.results = array of predictions returned by the model.
                   // Vision expresses classification results as VNClassificationObservation objects.
                   let topResult = results.first {
                    DispatchQueue.main.async {
                        self.prediction = // updates @state
                        "This looks like \(topResult.identifier)\nConfidence: \(Int(topResult.confidence * 100))%"
                    }
                }
            }
            
            let handler = VNImageRequestHandler(ciImage: ciImage) // the object that actually runs Vision requests
            try handler.perform([request])  //Run this list of Vision requests on the image(classification in our case)
